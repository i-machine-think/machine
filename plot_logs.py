...
def baseline(model):
    if 'baseline' in model and 'pre_rnn' in model:
            # and ('E16xH256' in model or 'E62xH256'  in model or 'E64xH512' in
            # model):
        return True
    return False
...
def plot_size_correlation():
    fig = log.plot_metric('seq_acc', restrict_model=baseline, restrict_data=heldout_tables,
                          data_name_parser=data_name_parser, color_group=color_size, eor=400)
...
def plot_pre_and_baseline():
    ...
    fig = log.plot_metric('nll_loss', restrict_model=best_pre_and_baseline, restrict_data=not_longer,
                          data_name_parser=data_name_parser, color_group=color_train, eor=-165)
    fig.savefig(
        '/home/dieuwke/Documents/papers/AttentionGuidance/figures/best_config_all_sets_loss.png')


def plot_val_loss():
    # plot accuracy of all validation sets for best configuration for learned
    # attention (pre) and baseline models to show overfitting
    fig = log.plot_metric('nll_loss', restrict_model=best_pre_and_baseline,
                          restrict_data=heldout_tables, data_name_parser=data_name_parser, color_group=color_train)
...
def plot_heldout_tables_all():
    ...
    fig = log.plot_metric('seq_acc', restrict_model=pre_and_baseline,
                          restrict_data=heldout_tables, color_group=color_baseline, eor=-165)
    fig.savefig(
        '/home/dieuwke/Documents/papers/AttentionGuidance/figures/all_models_heldout_tables.png')


log = LogCollection()
log.add_log_from_folder('chosens_dump', ext='.dump', name_parser=name_parser)

fig = log.plot_groups('nll_loss', restrict_model=ff_and_baseline, find_basename=find_basename,
                      find_data_name=find_data_name, restrict_data=not_longer, color_group=color_conditions, eor=-135)
fig.savefig(
    '/home/dieuwke/Documents/papers/AttentionGuidance/figures/lookup_loss_convergence.png')
